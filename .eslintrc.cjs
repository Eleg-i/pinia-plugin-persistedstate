...
const isCheckEffect = process.env.NODE_ENV === 'check-effect'
...
module.exports = {
  ...
  plugins: ['vue', 'jsx', '@typescript-eslint', 'jsdoc', 'import', 'tree-shaking'],
  root: true,
  // 默认可用的全局变量
  rules: {
    ...
    'no-console': [1, { allow: ['warn', 'error', 'info', 'debug', 'groupCollapsed', 'groupEnd'] }], //不能使用console
    ...
    'one-var': [2, { var: 'always', let: 'consecutive', const: 'never' }], //强制集体声明
    ...
    'vue/block-lang': [2, { script: { lang: 'ts' } }], // 要求 script 标签 lang 属性为 ts
    'vue/block-order': [
      1,
      {
        order: [
    ...
    'vue/no-multiple-template-root': 0, // 禁止多个根节点
    ...
    'vue/no-v-model-argument': 0, // 禁止在 v-model 指令中出现参数
